...
import com.hexaware.ftp88.persistence.LeaveDetailsDAO;
...
public class LeaveDetails {
  ...
  /**
   * leavestatus to store  leave status manager.
   */
  private LeaveStatus leaveStatusMgr;
  /**
   * leavestatus to store  leave status HR.
   */
  private LeaveStatus leaveStatusHR;
  /**
  ...
  public final int hashCode() {
    return Objects.hash(leaveId, empId, leaveSdate, leaveEdate, leaveNdays, leaveType, leaveStatusMgr,
    leaveStatusHR, leaveReason, leaveAppliedOn, leaveManagerComm);
  }
   ...
   * @param argLeaveStatusMgr to initialize Leave status.
   * @param argLeaveStatusHR to initialize Leave status.
  ...
  public LeaveDetails(final int argLeaveId, final int argEmpId, final Date argLeaveSdate,
      final Date argLeaveEdate, final int argLeaveNdays, final LeaveType argLeaveType,
      final LeaveStatus argLeaveStatusMgr,
      final LeaveStatus argLeaveStatusHR, final String argLeaveReason,
      final Date argLeaveAppliedOn, final String argLeaveManagerComm) {
    ...
    this.leaveStatusMgr = argLeaveStatusMgr;
    this.leaveStatusHR = argLeaveStatusHR;
   ...
   */
  public final LeaveStatus getLeaveStatusMgr() {
    return leaveStatusMgr;
  }

  /**
   *
   * @param argLeaveStatusMgr to set employee Leavbal.
   */
  public final void setLeaveStatusMgr(final LeaveStatus argLeaveStatusMgr) {
    this.leaveStatusMgr = argLeaveStatusMgr;
  }
   ...
   * @return this Employee's Leavbal.
   */
  public final LeaveStatus getLeaveStatusHR() {
    return leaveStatusHR;
  }

  /**
   *
   * @param argLeaveStatusHR to set employee Leavbal.
   */
  public final void setLeaveStatusHR(final LeaveStatus argLeaveStatusHR) {
    this.leaveStatusHR = argLeaveStatusHR;
  }
  ...
  public final boolean equals(final Object obj) {
    ...
    if (Objects.equals(leaveId, ld.leaveId)
        ...
        && Objects.equals(leaveStatusMgr, ld.leaveStatusMgr)
        && Objects.equals(leaveStatusHR, ld.leaveStatusHR)
...
/**
 ...
 /**
   ...
   * @return all employees' details
   */
  public static LeaveDetails[] leaveAll() {
...
/**
  ...
  public static String approveDenyHR(final int leaveId, final int empId,
    ...
    Employee emps = Employee.listById(empId);
    if (!emps.getDesignation().equals("HR")) {
      res = "You are not authorised to access this employee.";
      return res;
    }
    if (ld.getLeaveStatusMgr().equals(LeaveStatus.PENDING)) {
      res = "Not approved by manager";
      return res;
    } else if (lstatus.equals("YES") && ld.getLeaveStatusMgr().equals(LeaveStatus.APPROVED)
        && ld.getLeaveStatusHR().equals(LeaveStatus.PENDING)) {
      dbStatus = "APPROVED";
      res = "Leave Approved Successfully";
    } else if (lstatus.equals("YES") && ld.getLeaveStatusHR().equals(LeaveStatus.REJECTED)) {
      dbStatus = "APPROVED";
      res = "Leave Approved Successfully";
      edao().decrementMl(emplId, noOfDays);
    } else if (lstatus.equals("NO") && ld.getLeaveStatusHR().equals(LeaveStatus.APPROVED)) {
      dbStatus = "REJECTED";
      res = "Leave Rejected";
      edao().incrementML(emplId, noOfDays);
    } else if (lstatus.equals("NO")) {
      dbStatus = "REJECTED";
      res = "Leave Rejected";
      edao().incrementML(emplId, noOfDays);
    } else {
      res = "Already Rejected By manager.";
    }
    dao().commentHR(mgrcomments, dbStatus, leaveId);
    return res;
  }
...
/**
  ...
  public static String approveDeny(final int leaveId, final int empId, final String lStatus, final String mgrComments) {
    ...
    if (ld != null) {
      ...
      if (lStatus.equals("APPROVE") && ld.getLeaveType().equals(LeaveType.ML)) {
        dbStatus = "APPROVED";
        res = "Leave Approved Successfully By Manager";
        dao().comment(mgrComments, dbStatus, leaveId);
      } else if (lStatus.equals("DENY") && ld.getLeaveType().equals(LeaveType.ML)) {
        dbStatus = "REJECTED";
        edao().incrementML(emplId, noOfDays);
        dao().comment(mgrComments , dbStatus, leaveId);
        res = "Leave Rejected";
      } else if (lStatus.equals("APPROVE") && ld.getLeaveStatusMgr().equals(LeaveStatus.PENDING)) {
        dbStatus = "APPROVED";
        dao().comment(mgrComments, dbStatus, leaveId);
        res = "Leave Approved Successfully";
      } else if (lStatus.equals("DENY") && ld.getLeaveStatusMgr().equals(LeaveStatus.APPROVED)) {
        dbStatus = "REJECTED";
        edao().increment(emplId, noOfDays);
        dao().comment(mgrComments, dbStatus, leaveId);
        res = "Leave Rejected";
      } else if (lStatus.equals("APPROVE") && ld.getLeaveStatusMgr().equals(LeaveStatus.REJECTED)) {
        dbStatus = "APPROVED";
        edao().decrement(emplId, noOfDays);
        dao().comment(mgrComments, dbStatus, leaveId);
        res = "Leave Approved Successfully";
      } else {
        if (lStatus.equals("DENY") && ld.getLeaveStatusMgr().equals(LeaveStatus.PENDING)) {
          dbStatus = "REJECTED";
          edao().increment(emplId, noOfDays);
          dao().comment(mgrComments, dbStatus, leaveId);
          res = "Leave Rejected";
...
/**
  ...
  public static String applyLeave(final int empId,
    ...
    if (emplo != null) {
      ...
      long dif = 0;
      long updLeave = 0;
      String leaveStatusMGR = null;
      String leaveStatusHR = null;
      ...
      } else if (emplo.getMgrId() == 0) {
        leaveStatusMGR = "APPROVED";
        leaveStatusHR = "APPROVED";
        s = "Leave Applied Successfully";
      } else {
        s = "Leave Applied Successfully";
      }
      if (leaveType.equals("EL")) {
        leaveStatusMGR = "PENDING";
        leaveStatusHR = "APPROVED";
        dao().apply(empId, startDate, endDate, noOfdays,
                leaveType, leaveStatusMGR, leaveStatusHR, leaveReason, appliedOn);
        int bal = (int) updLeave;
        edao().decrement(empId, bal);

      }  else if (leaveType.equals("ML")) {
        leaveStatusMGR = "PENDING";
        leaveStatusHR = "PENDING";
        dao().apply(empId, startDate, endDate, noOfdays,
            leaveType, leaveStatusMGR, leaveStatusHR, leaveReason, appliedOn);
        int bal = (int) updLeave;
        edao().decrementMl(empId, bal);
        s = "Leave Applied Successfully";
      }
    } else {
      s = "invalid employee";
